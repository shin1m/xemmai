...
void t_writer::f_write(t_io* a_extension)
{
	...
	t_pvalue(v_stream).f_invoke(a_extension->f_symbol_write(), t_pvalue(v_buffer), f_global()->f_as(0), f_global()->f_as(v_p - p));
	v_p = p;
	v_n = buffer.f_size();
}
...
{
	return f_new<t_writer>(f_extension<t_io>(f_engine()->f_module_io()), false, a_stream, a_encoding);
}
...
void t_writer::f_write(t_io* a_extension, const t_pvalue& a_value)
{
	if (!v_stream) f_throw(L"already closed."sv);
	if (f_is<t_string>(a_value)) {
		f_write(a_extension, f_as<const t_string&>(a_value));
	} else {
		auto x = a_value.f_invoke(f_global()->f_symbol_string());
		f_check<t_string>(x, L"value");
		f_write(a_extension, f_as<const t_string&>(x));
	}
}
...
void t_writer::f_write_line(t_io* a_extension)
{
	...
	f_unshift(a_extension);
	t_pvalue(v_stream).f_invoke(a_extension->f_symbol_flush());
}

void t_writer::f_write_line(t_io* a_extension, const t_pvalue& a_value)
{
	if (!v_stream) f_throw(L"already closed."sv);
	if (f_is<t_string>(a_value)) {
		f_write(a_extension, f_as<const t_string&>(a_value));
...
void t_type_of<io::t_writer>::f_define(t_io* a_extension)
{
	t_define<io::t_writer, t_object>(a_extension, L"Writer"sv)
		...
		(a_extension->f_symbol_write(), t_member<void(io::t_writer::*)(t_io*, const t_pvalue&), &io::t_writer::f_write, t_with_lock_for_write>())
		(a_extension->f_symbol_write_line(),
			t_member<void(io::t_writer::*)(t_io*), &io::t_writer::f_write_line, t_with_lock_for_write>(),
			t_member<void(io::t_writer::*)(t_io*, const t_pvalue&), &io::t_writer::f_write_line, t_with_lock_for_write>()
		)
...
{
	return t_construct<false, const t_pvalue&, const t_string&>::t_bind<io::t_writer>::f_do(this, a_stack, a_n);
}
