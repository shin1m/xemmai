...
{
	return a_n > 0 ? t_type_of<t_string>::f__construct(f_global()->f_type<t_string>(), a_p, a_n) : t_scoped(f_global()->f_string_empty());
}
...
void t_type_of<t_string>::f_define()
{
	t_define<t_string, t_object>(f_global(), L"String"sv)
		...
		(f_global()->f_symbol_add(), t_member<t_scoped(*)(const t_value&, t_scoped&&), f__add>())
...
size_t t_type_of<t_string>::f_do_add(t_object* a_this, t_stacked* a_stack)
{
	...
	a_stack[0].f_construct(f__add(a_this, std::move(a0)));
	return -1;
}
