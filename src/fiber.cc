...
t_transfer t_fiber::f_instantiate(const t_transfer& a_callable, bool a_main, bool a_active)
{
	...
	object->v_pointer = new t_fiber(a_callable, a_main, a_active);
	return object;
}
...
void t_fiber::f_caught(t_object* a_object)
{
...
void t_type_of<t_fiber>::f_call(t_object* a_this, t_object* a_self, size_t a_n, t_stack& a_stack)
{
	if (a_n != 1) t_throwable::f_throw(L"must be called with an argument.");
	t_fiber* p = f_as<t_fiber*>(a_this);
