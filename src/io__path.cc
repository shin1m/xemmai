...
{
	t_define<portable::t_path, t_object>{a_library}
...
size_t t_type_of<portable::t_path>::f_do_divide(t_object* a_this, t_pvalue* a_stack)
{
	...
	a_stack[0] = a_this->f_type()->v_module->f_as<t_io>().f_as(f_as<const portable::t_path&>(a_this) / f_as<std::wstring>(a_stack[2]));
	return -1;
}
