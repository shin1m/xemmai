...
void t_object_get::f_generate(t_generator& a_generator, size_t a_stack, t_instruction a_instruction)
{
	...
	a_generator.f_operand(script.f_slot(t_scoped()));
	a_generator.f_operand(0);
	a_generator.f_at(this);
}
...
t_operand t_symbol_get::f_generate(t_generator& a_generator, size_t a_stack, bool a_tail, bool a_operand, bool a_clear)
{
	...
	if (!v_variable) {
		...
		a_generator.f_operand(a_stack);
		a_generator.f_operand(v_symbol);
		a_generator.f_at(this);
		if (a_clear) a_generator.f_emit_clear(a_stack);
		return a_stack;
	}
...
t_operand t_unary::f_generate(t_generator& a_generator, size_t a_stack, bool a_tail, bool a_operand, bool a_clear)
{
	...
	case t_operand::e_tag__LITERAL:
		...
		break;
	case t_operand::e_tag__VARIABLE:
		a_generator.f_operand(operand.v_index);
		break;
	}
	a_generator.f_at(this);
	if (a_clear) a_generator.f_emit_clear(a_stack);
...
t_operand t_binary::f_generate(t_generator& a_generator, size_t a_stack, bool a_tail, bool a_operand, bool a_clear)
{
	...
	switch (left.v_tag) {
	case t_operand::e_tag__INTEGER:
		a_generator.f_operand(left.v_integer);
		break;
	case t_operand::e_tag__FLOAT:
		a_generator.f_operand(left.v_float);
		break;
	case t_operand::e_tag__LITERAL:
		a_generator.f_operand(left.v_value);
		break;
	case t_operand::e_tag__VARIABLE:
		a_generator.f_operand(left.v_index);
		break;
	}
	switch (right.v_tag) {
	case t_operand::e_tag__INTEGER:
		a_generator.f_operand(right.v_integer);
		break;
	case t_operand::e_tag__FLOAT:
		a_generator.f_operand(right.v_float);
		break;
	case t_operand::e_tag__LITERAL:
		a_generator.f_operand(right.v_value);
		break;
	case t_operand::e_tag__VARIABLE:
		a_generator.f_operand(right.v_index);
		break;
	}
	a_generator.f_at(this);
	if (a_clear) a_generator.f_emit_clear(a_stack);
...
{
	v_scope = &a_scope;
	size_t stack = a_scope.v_privates.size();
	t_scoped code = t_code::f_instantiate(v_module, true, false, stack, a_scope.v_shareds, 0, 0);
	...
	ast::f_generate_block_without_value(*this, stack, a_scope.v_block);
