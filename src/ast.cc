	...
	if (a_clear && !v_variable.v_shared) {
		...
		case t_operand::c_tag__LITERAL:
			...
			case c_tag__NULL:
				...
				break;
			case c_tag__TRUE:
				a_emit << c_instruction__TRU << v_variable.v_index;
				break;
			default:
...
t_operand t_boolean::f_emit(t_emit& a_emit, bool a_tail, bool a_operand, bool a_clear)
{
	...
	if (a_tail) {
		a_emit.f_emit_safe_point(this);
		(a_emit << static_cast<t_instruction>(c_instruction__RETURN_NUL + v_value)).f_push();
	} else {
		(a_emit << static_cast<t_instruction>(c_instruction__NUL + v_value) << a_emit.v_stack).f_push();
	}
	a_emit.f_at(this);
	return {};
}
