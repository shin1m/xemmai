		...
		case t_lexer::c_token__LEFT_PARENTHESIS:
			{
				...
				if ((!v_lexer.f_newline() || v_lexer.f_indent() > indent) && v_lexer.f_token() != t_lexer::c_token__RIGHT_PARENTHESIS) call->v_expand = f_expressions(a_depth + 1, a_head, indent, call->v_arguments);
	...
	case t_lexer::c_token__LEFT_BRACKET:
		{
			...
			if ((!v_lexer.f_newline() || v_lexer.f_indent() > indent) && v_lexer.f_token() != t_lexer::c_token__RIGHT_BRACKET) call->v_expand = f_expressions(a_depth + 1, a_head, indent, call->v_arguments);
		...
		case t_lexer::c_token__LEFT_PARENTHESIS:
			{
				...
				if ((!v_lexer.f_newline() || v_lexer.f_indent() > a_indent) && v_lexer.f_token() != t_lexer::c_token__RIGHT_PARENTHESIS) call->v_expand = f_expressions(a_depth + 1, a_head, a_indent, call->v_arguments);
				if ((!v_lexer.f_newline() || v_lexer.f_indent() >= a_indent) && v_lexer.f_token() == t_lexer::c_token__RIGHT_PARENTHESIS) v_lexer.f_next();
	...
	case t_lexer::c_token__FOR:
		{
			...
			if (v_lexer.f_token() != t_lexer::c_token__SEMICOLON) f_expressions(a_depth + 1, a_head, node->v_initialization);
			if (v_lexer.f_token() != t_lexer::c_token__SEMICOLON) f_throw(L"expecting ';'."sv);
			v_lexer.f_next();
			size_t varies = v_scope->v_privates.size();
			if (v_lexer.f_token() != t_lexer::c_token__SEMICOLON) node->v_condition = f_expression(a_depth + 1, a_head);
			if (v_lexer.f_token() != t_lexer::c_token__SEMICOLON) f_throw(L"expecting ';'."sv);
			v_lexer.f_next();
