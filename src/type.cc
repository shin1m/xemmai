...
void t_type::f_instantiate(t_object* a_class, size_t a_n, t_stack& a_stack)
{
	...
	object.f_get(f_global()->f_symbol_initialize()).f_call_and_return(t_value(), a_n, a_stack);
...
t_transfer t_type::f_get(const t_value& a_this, t_object* a_key)
{
	...
	t_transfer value;
	if (reinterpret_cast<size_t>(&*a_this) >= t_value::e_tag__OBJECT) {
		portable::t_scoped_lock_for_read lock((*a_this).v_lock);
		field = (*a_this).v_fields.f_find<t_object::t_hash_traits>(a_key);
...
{
	a_this->f_get(f_global()->f_symbol_hash()).f_call(t_value(), 0, a_stack);
}

void t_type::f_call(t_object* a_this, const t_value& a_self, size_t a_n, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_call()).f_call(t_value(), a_n, a_stack);
}

void t_type::f_get_at(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_get_at()).f_call(t_value(), 1, a_stack);
}

void t_type::f_set_at(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_set_at()).f_call(t_value(), 2, a_stack);
}

void t_type::f_plus(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_plus()).f_call(t_value(), 0, a_stack);
}

void t_type::f_minus(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_minus()).f_call(t_value(), 0, a_stack);
}

void t_type::f_not(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_not()).f_call(t_value(), 0, a_stack);
}

void t_type::f_complement(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_complement()).f_call(t_value(), 0, a_stack);
}

void t_type::f_multiply(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_multiply()).f_call(t_value(), 1, a_stack);
}

void t_type::f_divide(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_divide()).f_call(t_value(), 1, a_stack);
}

void t_type::f_modulus(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_modulus()).f_call(t_value(), 1, a_stack);
}

void t_type::f_add(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_add()).f_call(t_value(), 1, a_stack);
}

void t_type::f_subtract(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_subtract()).f_call(t_value(), 1, a_stack);
}

void t_type::f_left_shift(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_left_shift()).f_call(t_value(), 1, a_stack);
}

void t_type::f_right_shift(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_right_shift()).f_call(t_value(), 1, a_stack);
}

void t_type::f_less(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_less()).f_call(t_value(), 1, a_stack);
}

void t_type::f_less_equal(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_less_equal()).f_call(t_value(), 1, a_stack);
}

void t_type::f_greater(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_greater()).f_call(t_value(), 1, a_stack);
}

void t_type::f_greater_equal(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_greater_equal()).f_call(t_value(), 1, a_stack);
}

void t_type::f_equals(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_equals()).f_call(t_value(), 1, a_stack);
}

void t_type::f_not_equals(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_not_equals()).f_call(t_value(), 1, a_stack);
}

void t_type::f_and(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_and()).f_call(t_value(), 1, a_stack);
}

void t_type::f_xor(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_xor()).f_call(t_value(), 1, a_stack);
}

void t_type::f_or(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_or()).f_call(t_value(), 1, a_stack);
}

void t_type::f_send(t_object* a_this, t_stack& a_stack)
{
	a_this->f_get(f_global()->f_symbol_send()).f_call(t_value(), 1, a_stack);
}
