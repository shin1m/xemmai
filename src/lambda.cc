...
void t_type_of<t_lambda>::f_instantiate(t_object* a_class, size_t a_n)
{
	t_throwable::f_throw(L"uninstantiatable.");
}

void t_type_of<t_lambda>::f_call(t_object* a_this, const t_value& a_self, size_t a_n)
{
	t_lambda& p = f_as<t_lambda&>(a_this);
	t_code& code = f_as<t_code&>(p.v_code);
	if (a_n != code.v_arguments) t_throwable::f_throw(L"invalid number of arguments.");
	t_fiber::t_context::f_push(p.v_code, p.v_scope, a_self, a_n);
}

void t_type_of<t_lambda>::f_get_at(t_object* a_this)
{
	t_native_context context;
	t_stack* stack = f_stack();
	t_transfer a0 = stack->f_pop();
	stack->f_return(t_method::f_instantiate(a_this, a0));
	context.f_done();
}
