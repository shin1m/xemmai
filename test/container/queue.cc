...
std::wstring t_queue::f_string() const
{
	...
	std::wstring s;
	if (v_head.f_object()) {
		t_object* pair = f_as<t_pair&>(v_head).v_next.f_object();
		while (true) {
			t_scoped p = f_as<t_pair&>(pair).v_value.f_get(f_global()->f_symbol_string())();
			s += f_is<std::wstring>(p) ? f_as<const std::wstring&>(p) : L"<unprintable>";
			s += L'\n';
			if (pair == v_head.f_object()) break;
			pair = f_as<t_pair&>(pair).v_next.f_object();
		}
	}
	return s;
}
...
{
	portable::t_scoped_lock_for_write lock(v_lock);
	if (!v_head.f_object()) t_throwable::f_throw(L"empty queue.");
