...
#define XEMMAI__MACRO__T_OVERLOADN_CLOSE__ XEMMAI__MACRO__REPEAT(XEMMAI__MACRO__T_OVERLOADN_CLOSE, XEMMAI__MACRO__N)
	...
	t_define& operator()(const t_transfer& a_name, const t_member<T_function, A_function>&, XEMMAI__MACRO__CONST_T_OVERLOADN__)
	{
		v_type->f_put(a_name, v_extension->f_function(
			t_overload<t_member<T_function, A_function>
	...
	template<typename T_function, T_function A_function, XEMMAI__MACRO__TYPENAME_T_OVERLOADN__>
	t_define& operator()(const std::wstring& a_name, const t_member<T_function, A_function>& a_member0, XEMMAI__MACRO__CONST_T_OVERLOADN_A_OVERLOADN__)
	{
		return (*this)(t_symbol::f_instantiate(a_name), a_member0, XEMMAI__MACRO__A_OVERLOADN__);
	}
	template<typename T_function, T_function A_function, XEMMAI__MACRO__TYPENAME_T_OVERLOADN__>
	t_define& operator()(const t_transfer& a_name, const t_static<T_function, A_function>&, XEMMAI__MACRO__CONST_T_OVERLOADN__)
	{
		v_type->f_put(a_name, v_extension->f_function(
