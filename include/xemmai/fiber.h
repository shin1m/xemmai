...
struct t_context
{
	...
	t_context(t_lambda* a_lambda, t_pvalue* a_stack) : v_pc(a_lambda->v_instructions), v_base(a_stack + 2), v_lambda(a_lambda)
	{
		v_previous = f_stack();
		f_stack__(v_base + v_lambda->v_size);
	}
	template<typename T>
	...
	{
		f_return(v_base[v_lambda->v_privates]);
	}
...
struct t_debug_context : t_context
{
	...
	t_debug_context(t_lambda* a_lambda, t_pvalue* a_stack) : t_context(a_lambda, a_stack)
	{
