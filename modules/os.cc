...
{
	return t_construct_with<t_object*(*)(t_type*, std::wstring_view, const t_pvalue&, const t_pvalue&, const t_pvalue&), [](auto a_class, auto a_file, auto a_arguments, auto a_environments, auto a_ios)
	{
		auto size = [](auto& a_xs)
		{
			static size_t index;
		...
		std::vector arguments0{{portable::f_convert(a_file)}};
		...
		closes[0].clear();
		return a_class->template f_new<t_child>(pid, pipes);
	}>::f_do(this, a_stack, a_n);
}
#endif
